...
$check = lookup_url($url, $_SESSION['urllist']);

if (empty($url))
{
    $_SESSION['error_urls'] = "<font color=#FF0000>URL text area is empty!</font>";
    header('Location: search-url.php');
} elseif (!empty($url) && $check == -1) {
    $_SESSION['error_urls'] = "<font color=#FF0000>Illegal characters in URL list!</font>";
    header('Location: search-url.php');
} elseif (!empty($url) && $check == -2) {
    $_SESSION['error_urls'] = "<font color=#FF0000>URL list not selected</font>";
    header('Location: search-url.php');
} elseif (!empty($url) && $check == 0) { // $_SESSION['URL_MANAGER_LOOKUP_URL_STATUS'] is set inside the function
    header('Location: search-url.php');
}
?>
